...
echo "$($PSQL "SELECT name FROM (SELECT * FROM teams INNER JOIN games ON teams.team_id = games.winner_id UNION SELECT * FROM teams INNER JOIN games ON teams.team_id = games.opponent_id) AS derivedTable WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
